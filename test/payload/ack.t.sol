...
contract AckContract {
    ...
    function getIncentivizedAckPacketBytes(
        IncentivizedAckPacket calldata ack
    ) external pure returns (bytes memory) {
        bytes memory relayerBytes = bytes(ack.relayer);
        return
            abi.encodePacked(
                ack.success,
                uint32(relayerBytes.length),
                relayerBytes,
                ack.data
            );
    }
...
contract AckTest is Test {
    ...
    bytes32 ackPacket1Hash =
        hex"863b46bf0508a780d8954b8f22b37e91a76c6427a2dcdff98076f977adda55b7";

    bytes ackPayload2 =
        hex"00000000147e5f4552091a69125d5dfcb7b8c2659029395bdf61636b2d64617461";
    ...
    bytes32 ackPacket2Hash =
        hex"5037447b5f5cf30dac6895cca07855054abe38b989e16837ef3b8d929f523e0a";
    ...
    function testAckPacketBytes() public {
        ...
        assertEq(
            ackPacketBytes,
            ackPayload1,
            "ackPacketBytes should be equal to ackPayload1"
        );
        assertEq(
            ackPacket1Hash,
            sha256(ackPacketBytes),
            "ackPacket1Hash should be equal to keccak256(ackPacketBytes)"
        );
    }
    ...
    function testIncentivizedAckPacketBytes() public {
        bytes memory ackPacketBytes = ackContract.getIncentivizedAckPacketBytes(
            ackPacket2
        );
        ...
        require(
            ackPacket2.relayer.length == 20,
            "invalid relayer address length"
        );
        address relayer = address(bytes20(ackPacket2.relayer));
        assertEq(
            relayer,
            vm.addr(0x01),
            "relayer should be equal to vm.addr(0x01)"
        );

        assertEq(
            ackPacketBytes,
            ackPayload2,
            "ackPacketBytes should be equal to ackPayload2"
        );
        assertEq(
            ackPacket2Hash,
            sha256(ackPacketBytes),
            "ackPacket2Hash should be equal to keccak256(ackPacketBytes)"
        );
    }
}
