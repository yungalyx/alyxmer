...
import "@openibc/contracts/core/Dispatcher.sol";
import "@openibc/contracts/interfaces/ProofVerifier.sol";
import {UniversalChannelHandler} from "@openibc/contracts/core/UniversalChannelHandler.sol";
import {DAONFT} from "../contracts/DAONFT.sol";
import {IbcMiddleware} from "@openibc/contracts/interfaces/IbcMiddleware.sol";
import {GeneralMiddleware} from "@openibc/contracts/base/GeneralMiddleware.sol";
...
contract VirtualChain is Test, IbcEventsEmitter {
    ...
    constructor(uint256 seed, string memory portPrefix) {
        ...
        mars = new DAONFT(address(ucHandler));
        daonft = new DAONFT(address(ucHandler));
        // initialize portIds for counterparty chains
        address[3] memory portContracts = [
            address(ucHandler),
            address(mars),
            address(daonft)
        ];
        for (uint256 i = 0; i < portContracts.length; i++) {
            portIds[portContracts[i]] = string(
                abi.encodePacked(portPrefix, toHexStr(portContracts[i]))
            );
        }
    ...
    function getVirtualChainData()
        external
        view
        returns (VirtualChainData memory)
    {
        return
            VirtualChainData(
                dispatcher,
    ...
    function finishHandshake(
        ...
        vm.prank(address(remoteChain));
        remoteChain.channelOpenAckOrConfirm(
            remoteEnd,
            this,
            localEnd,
            setting,
    ...
    function channelOpenInit(
        ...
        require(
            bytes(cpPortId).length > 0,
            "channelOpenTry: portId does not exist"
        );
    ...
    function channelOpenTry(
        ...
        bytes32 cpChanId = remoteChain.channelIds(
            address(remoteEnd),
            address(localEnd)
        );
        require(
            cpChanId != bytes32(0),
            "channelOpenTry: channel does not exist"
        );
    ...
    function channelOpenAckOrConfirm(
        ...
        require(
            chanId != bytes32(0),
            "channelOpenAckOrConfirm: channel does not exist"
        );
        ...
        require(
            bytes(cpPortId).length > 0,
            "channelOpenAckOrConfirm: counterparty portId does not exist"
        );
    ...
    function localEndToCounterparty(
        address localEnd
    ) external view returns (CounterParty memory) {
        return
            CounterParty(
                portIds[localEnd],
                channelIds[localEnd][address(this)],
                ""
            );
    }
    ...
    function newChannelId() external returns (bytes32) {
        bytes memory channelId = abi.encodePacked(
            "channel-",
            Strings.toString(_seed)
        );
        _seed += 1;
    ...
    function newConnectionId() internal returns (string memory) {
        string memory connectionId = string(
            abi.encodePacked("connection-", Strings.toString(_seed))
        );
        _seed += 1;
        return connectionId;
    }
    ...
    function toHexStr(address addr) internal pure returns (bytes memory) {
        bytes memory addrWithPrefix = abi.encodePacked(
